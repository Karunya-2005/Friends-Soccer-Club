...
A static site for a community football club: hero, about, head coach profile,
programs (age groups), schedule, contact-coach CTA, and a registration form.
...
├── script.js                # mobile nav, scroll animations, stat count-up,
│                            # age-based guardian fields, form submit
├── assets/
│   └── coach-george-robert.jpeg   # head coach photo
└── README.md
```
...
The `#coach` section in `index.html` contains:

- **Profile** — Coach George Robert.W, Head Coach, 20+ years experience,
  Chennai (Chrompet), contact +91 98402 52417. Photo lives in `assets/`.
- **Stat cards** — years / students / matches / championships, with a
  count-up animation when scrolled into view.
- **Weekly training schedule** — animated day cards (Mon–Fri 5:30–7:00 AM,
  Sat 6:00–8:00 AM, Sun holiday).
- **Contact CTA** — the `#contact-coach` section before Register, with
  "Register now" and "Call coach" (tel: link) buttons.

To update any coach details, edit the `#coach` and `#contact-coach` sections
in `index.html`. To swap the photo, replace `assets/coach-george-robert.jpeg`
(keep the same filename, or update the `<img src>` in `index.html`).
...
- `index.html`: email and social links (footer), founding year and stats
  (hero ticker + About section), fixture list (Schedule), coach details
  (`#coach` section).
- Club colors/fonts live as CSS variables at the top of `styles.css` under
  `:root` — the coach section's tricolor accent uses `--saffron` alongside
  the existing turf green and gold.
